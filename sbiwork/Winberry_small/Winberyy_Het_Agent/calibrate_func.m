function [bbeta] = calibrate_func(vecin)
...
global bbeta ssigma aaBar aalpha ddelta vEpsilonGrid aggEmployment uDuration ...
    mmu rrhoTFP ssigmaTFP ttau mu_l ssigmaMeas ssigmaMeasmicro;
  
% Preferences
bbeta = vecin(1);%.96;                    % discount factor (annual calibration)
ssigma = 1;                     % coefficient of relative risk aversion
aaBar = vecin(8);                      % borrowing constraint
...
mu_l = vecin(2);%-.25; % Roughly calibrated to Piketty, Saez & Zucman (QJE 2018) Table I, log of P90-P20 ratio, post-tax

% Measurement error std. dev. of observed log output
ssigmaMeas = [vecin(3), vecin(4), vecin(5), vecin(6)];%vecin(2);%0.02;

ssigmaMeasmicro = vecin(7);
